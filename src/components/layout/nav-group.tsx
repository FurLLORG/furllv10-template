...
import { navLinkActive } from '@/lib/client-menu'
...
export function NavGroup({ title, items }: NavGroupProps) {
  ...
  return (
    <SidebarGroup>
      ...
      <SidebarMenu>
        {items.map((item) => {
          const key = `${item.title}-${item.url ?? item.items?.map((i) => i.url).join(',')}`
...
function NavIcon({ item }: { item: NavItem }) {
  if (item.iconClass) {
    return <i className={`iconfont ${item.iconClass} text-[15px] leading-none`} />
  }
...
/**
 * 链接与当前地址是否匹配：
 * 菜单项带 search（如 product.htm?m=2403）时，要求当前地址 query 包含相同键值，
 * 避免多个同为 /product.htm 的模块菜单（不同 ?m=）同时高亮
 */
function checkIsActive(href: string, item: NavItem, mainNav = false) {
  return (
    href === item.url || // /endpint?search=param
    navLinkActive(href, item) || // endpoint（含 search 键值匹配，如 product.htm?m=2403）
    !!item?.items?.filter((i) => navLinkActive(href, i)).length || // if child nav is active
    (mainNav &&
      href.split('/')[1] !== '' &&
