...
export default function Strands({
  ...
  useEffect(() => {
    ...
    gl.canvas.style.border = '0'
    gl.canvas.style.outline = '0'
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uColors: { value: buildPalette(propsRef.current.colors as string[]) },
        uColorCount: { value: Math.min((propsRef.current.colors as string[]).length, MAX_COLORS) },
        uStrandCount: { value: Math.min(propsRef.current.count as number, MAX_STRANDS) },
        uSpeed: { value: speed },
    ...
    function resize() {
      ...
      // 强制 canvas 用 CSS 100% 铺满自适应容器，仅绘制缓冲跟随容器尺寸
    ...
    const update = (t: number) => {
      ...
      const curColors = (current.colors as string[]) || []
      program.uniforms.uTime.value = t * 0.001
      program.uniforms.uColors.value = buildPalette(curColors)
      program.uniforms.uColorCount.value = Math.min(curColors.length, MAX_COLORS)
      program.uniforms.uStrandCount.value = Math.min(
        Math.max(Math.round(current.count as number), 1),
        MAX_STRANDS,
      )
      ...
      if (current.glass) {
        ...
        glassProgram.uniforms.uRadius.value = 0.46 * (current.glassSize as number)
